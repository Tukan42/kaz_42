...
window.onload = function() {
    ...
    // FORCE INIT ALL
    setTimeout(() => {
        if(typeof initKeno === 'function') initKeno();
        if(typeof updatePlinkoRisk === 'function') updatePlinkoRisk();
        if(typeof initTower === 'function') initTower();
    }, 100);
};
...
function updateBalanceUI() {
    const el1 = document.getElementById('balance-display');
    const el2 = document.getElementById('profile-balance-view');
    if(el1) el1.innerText = balance;
...
function adjustBet(id, amt) { 
    let el = document.getElementById(id); 
    let val = (parseInt(el.value)||0)+amt;
    if(val>balance) val=balance; el.value=val; 
}
...
function updateStats(win) { 
    stats.games++; if(win) stats.wins++; else stats.losses++; 
    const elTotal = document.getElementById('stat-total');
    if(elTotal) {
        elTotal.innerText=stats.games;
...
function resetRocketState() {
    ...
    isGameActive = false;
    const btnPlay = document.getElementById('btn-rocket-play');
    if(btnPlay) {
